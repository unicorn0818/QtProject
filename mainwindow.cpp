    ...
    ui(new Ui::MainWindow)
{
    ...
    p_wave_reout_ZJ = new wave_reout_ZJ();
   // p_power_test=new power_test();
    //p_test_bll=new test_bll();
    ui->tabWidget->addTab(p_wave_reout_ZJ,tr("负荷仿真"));
...
//    ui->tabWidget->addTab(p_wave_reout,tr("长录波回放"));
    ...
    //ui->tabWidget->tabBar()->setStyleSheet("QTabBar::tab{width:70px;}");
    ...
    //正泰二次开发
    connect(p_config_set,SIGNAL(send_to_TRJ(QStringList,int)),p_wave_reout_ZJ,SLOT(recvOrder(QStringList,int)),Qt::QueuedConnection);
    connect(p_wave_reout_ZJ,SIGNAL(sendBack(bool,int)),p_config_set,SLOT(recvBack(bool,int)),Qt::QueuedConnection);

    //connect(p_wave_reout,SIGNAL(send_out(QString)),P_NET,SLOT(send_byte(QString)),Qt::QueuedConnection);
    connect(p_wave_reout_ZJ,SIGNAL(send_out(QString)),P_NET,SLOT(send_byte(QString)),Qt::QueuedConnection);
    connect(p_wave_reout_ZJ,SIGNAL(setNowNum(int)),P_NET,SLOT(setNowNum(int)),Qt::QueuedConnection);
...
//    connect(P_NET,SIGNAL(send_over()),p_wave_reout,SLOT(recv_over()),Qt::QueuedConnection);
    connect(P_NET,SIGNAL(send_over()),p_wave_reout_ZJ,SLOT(recv_over()),Qt::QueuedConnection);

    //connect(p_wave_reout,SIGNAL(send_stop()),P_NET,SLOT(recv_stop()),Qt::QueuedConnection);
    connect(p_wave_reout_ZJ,SIGNAL(send_stop()),P_NET,SLOT(recv_stop()),Qt::QueuedConnection);
    //connect(P_NET,SIGNAL(send_index(int)),p_wave_reout,SLOT(recv_index(int)),Qt::QueuedConnection);
    connect(P_NET,SIGNAL(send_index(int)),p_wave_reout_ZJ,SLOT(recv_index(int)),Qt::QueuedConnection);
    ...
    connect(p_config_set,SIGNAL(m_reset(int)),this,SLOT(reset_size(int)));
    //用来防止发送重复的信息
    connect(p_wave_reout_ZJ,SIGNAL(disConnect(int,bool)),this,SLOT(getDisconnect(int,bool)));
    //connect(p_wave_reout,SIGNAL(disConnect(int,bool)),this,SLOT(getDisconnect(int,bool)));
    m_pSeriThread->start();
    timer=new QTimer;
...
//    send_order1(w,ww);
//}
void MainWindow::getDisconnect(int type,bool status)
{
    if(type == 1&&status==true)
    {
        disconnect(P_NET,SIGNAL(send_over()),p_wave_reout,SLOT(recv_over()));
        disconnect(P_NET,SIGNAL(send_index(int)),p_wave_reout,SLOT(recv_index(int)));
    }
    ...
    else if(type == 0&&status==true)
    {
        disconnect(P_NET,SIGNAL(send_over()),p_wave_reout_ZJ,SLOT(recv_over()));
        disconnect(P_NET,SIGNAL(send_index(int)),p_wave_reout_ZJ,SLOT(recv_index(int)));
    }
...
void MainWindow::reset_size(int type)
{
    if(type == 1)
        showNormal();
    else

	...
	enum WAVE_TYPE_DEF
{
	...
	private:
		...
		enum COMMAND_TYPE
		{
            ...
            SATRT_BIG_WAVE_REOUT        =0xBD,      //大数据录波启动
            ...
            TEST_POWER_U                =0xF7,      //测试蓄电池电压
            TEST_POWER_R                =0xF8,      //测试蓄电池内阻
            READ_POWER_U_R              =0xF9,      //读取蓄电池电压和内阻
            START_POWER_OUT_ELE         =0xFA,      //开始电池放电测试
            STOP_POWER_OUT_ELE           =0xFB,      //停止电池放电或放电结束
	...
	signals:
        ...
        void send_to_hrenter(const QByteArray &last,const QByteArray &now);
        void send_to_any_wave(const QByteArray &last,const QByteArray &now);
        void send_to_active_wave(const QByteArray &last,const QByteArray &now);
        void send_to_adjust_energy(const QByteArray &last,const QByteArray &now);
        void send_to_reout_wave(const QByteArray &last,const QByteArray &now);
        void send_to_wave_reout(const QByteArray &last,const QByteArray &now);
        void send_to_power_test(const QByteArray &last,const QByteArray &now);
        void send_to_bll_test(const QByteArray &last,const QByteArray &now);
        void send_to_Ele_Load_Sim(const QByteArray &last,const QByteArray &now);
	...
	public://与协议文本对应，用于单路同步等待应答 
        ...
        //单相写谐波
        bool write_one_harmonic(quint8 pipe, const PhaseOneHrDef data[], PAGE page);
        ...
        bool big_wave_reout(QByteArray arry,PAGE page);
        //校准部分
        ...
        //蓄电池测试部分
        bool test_power_u(PAGE page);
        bool test_power_r(PAGE page);
        bool read_power_u_r(PAGE page);
        bool start_power_out_ele(float i,float time,float u,PAGE page);
        bool stop_power_out_ele(PAGE page);
	...
	public://铺助函数
        ...
        qint8 getCalculatorValue(char *f,int type);
        double get_value(double time,int type);
        void test_bll(QByteArray);
        qint32 float_to_quint(float data);
	...
	private:
        ...
        char sufexp[3][EXP_MAX_LEN];//suffix expression 后缀表达式
        char buffer[3][EXP_MAX_LEN];//buffer

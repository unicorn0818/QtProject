...
qint8 XL803Proto::getCalculatorValue(char *f,int type)
{
    ...
    int i, j;//循环变量
    memset(sufexp[type],0,EXP_MAX_LEN);//初始化
    memset(buffer[type],0,EXP_MAX_LEN);
    sufexp[type][EXP_MAX_LEN-1]='\0';//末尾设为'\0'
    ...
    for(int i=0;i<strlen(sufexp[type]);i++)
        qDebug()<<"sufexp"<<sufexp[type][i++];
    return 1;
}

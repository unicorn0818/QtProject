...
#define EXP_MAX_LEN 1001
#define INF 1000000
#define LIM_ZERO 0.000001
...
void push_num(double f);//数字栈压栈
double pop_num(void);//数字栈弹栈
void push_op(char c);//符号栈压栈
char pop_op(void);//符号栈弹栈
...
void XL803Proto::readFromPort(const QByteArray &rb,const QByteArray &wB,XL803::PAGE page)
{
    ...
    case Config_set:
        ...
        break;
    case List_out:
        emit send_to_list_out(wB,rb);
        break;
    case STATE_LIST:
        emit send_to_state_out(wB,rb);
        break;
    case ACTIVE_WAVE:
...
bool XL803Proto::write_one_harmonic(quint8 pipe, PhaseOneHrDef data[HIGHT_HAR_TIMES],PAGE page)
{
    XL803_funcIn();
    quint8 hrCount = HIGHT_HAR_TIMES;
    *ms_data=pipe;
    pProtoHrHightWriteDefOne pD = (pProtoHrHightWriteDefOne)(ms_data+1);

    memset(pD,0,sizeof(ProtoHrHightWriteDefOne));
    for(int i = 0; i < hrCount; i ++)
    {
        pD->Radio[i] = data[i].radio / 100.0 * 0x6666;
        pD->Phase[i] = data[i].phase * 100.0;
    }
    ms_dSize = sizeof(ProtoHrHightWriteDefOne);
...
quint32 XL803Proto::float_to_quint(float data)
{
    quint32 new_data=0;
    for(int i=0;i<4;i++)
    {
...
float XL803Proto::quint_to_float(quint32 data)
{
    float new_data=0;
    for(int i=0;i<4;i++)
    {
...
void XL803Proto::sin_insert(QVector<M_point> in_point,QVector<M_point> &out_point,int point_num,double len)
{
    ...
    for(int i=0;i<in_point.count()-1;i++)
    {
        if(i==0)
        {
            ...
            if(last_flag&&!next_flag)//0~PI/2
            {
                type=0;
            }else if(!last_flag&&!next_flag)//PI/2~PI
            {
                type=1;
            }else if(!last_flag&&next_flag)//PI~(PI+PI/2)
            {
                type=2;
            }else if(last_flag&&next_flag)//(PI+PI/2)~2PI
            {
                type=3;
            }
            last_type=type;
        ...
        int all_count=point_num*((p_end.x-p_start.x)/len);
        switch (type) {
        case 0:
            A=(qAbs(p_end.y-p_start.y));
            b=p_end.y-A;
            t=0;
            break;
        case 1:
            A=(qAbs(p_start.y-p_end.y));
            b=p_start.y-A;
            t=p_end.x-p_start.x;
            break;
        case 2:
        ...
        case 3:
            A=(qAbs(p_start.y-p_end.y));
            b=p_start.y+A;
            t=(p_end.x-p_start.x)*3;
            break;
        default:
            break;
        }
        p_now.x=p_start.x;
        for(int i=0;i<all_count;i++,p_now.x+=step,t+=step)
        {
            p_now.y=A*sin(2*M_PI*(1/((p_end.x-p_start.x)*4))*t)+b;
            out_point<<p_now;
        }
    }
}*/
...
quint8 XL803Proto::getCalculatorValue(char *f,double &value)
{
    char exp[EXP_MAX_LEN];	//expression 表达式
    char sufexp[EXP_MAX_LEN];//suffix expression 后缀表达式
    char buffer[EXP_MAX_LEN];//buffer
    int len;//表达式长度
    int precision;//控制精度
    int biginteger;//控制是否大整数模式
    double angle;//控制弧度与角度转换
    int index, index_suf, index_buf;//指示表达式当前位置
    int i, j;//循环变量
    ...
    if (!biginteger)
    {
        ...
        for (i = 0; i < (int)strlen(buffer); i++)//全部转为小写
        ...
        for (i = 0, j = 0; i <= (int)strlen(buffer); i++)
        {
            ...
            else if (buffer[i] >= 'a' && buffer[i] <= 'z')//说明是函数或者是常数
            {
                if (j > 0)
                    if (is_num(exp[j-1]))
                        exp[j++]='*';
                //常数或函数前的*可以省略
                if (buffer[i] == 'e') {exp[j++]='E'; continue;}//常数E
                char tmp[5];
        ...
        while (index < len)
        {
            ...
            while ((index < len) && is_op(exp[index]))
            {
                ...
                if (c == ')')
                {
                    while (op_stack.op[op_stack.top] != '(')//所有（）间的元素出栈
                    {
                        sufexp[index_suf++] = pop_op();
                        sufexp[index_suf++] = ' ';
                    }
                    pop_op();//删除（
                }
                ...
                {
                    while (op_stack.top >= 0 && oplevel(c) <= oplevel(op_stack.op[op_stack.top]))
                    {
                        //不停弹栈输出，直到栈顶元素优先级比c低或为(为止
                        if (op_stack.op[op_stack.top]== '(')	break;
                        sufexp[index_suf++] = pop_op();
                        sufexp[index_suf++] = ' ';
                    }
                    push_op(c);
        ...
        while (index_suf < len)
        {
            ...
            while ((index_suf < len) && is_num(sufexp[index_suf]))
                buffer[index_buf++] = sufexp[index_suf++];
            if (buffer[index_buf-1] == 'P')
                push_num(PI);
            else if (buffer[index_buf-1] == 'E')
                push_num(E);
            else
            {
                buffer[index_buf] = '\0';//将字符串转为double
                push_num(atof(buffer));
            }
            index_suf++;//跳过空格
            //处理符号
            while ((index_suf < len) && is_op(sufexp[index_suf]))
            {
                char c = sufexp[index_suf];
                double f1, f2;//tmp
                int t;//tmp
                index_suf += 2;	//跳过空格
                switch (c)
                ...
                case '!':
                    ...
                    if (f1 - t != 0.0 || f1 < 0)//f不是整数或者是负数
                    {
                        ...
                        push_num((double)fac(t));
                    break;
                case 's'://sin
                    push_num(sin(pop_num()*angle));
                    break;
                case 'c'://cos
                    push_num(cos(pop_num()*angle));
                    break;
                case 't'://tan
                    f2 =pop_num();
                    f1 = tan(f2*angle);
                ...
                case 'o'://cot
                    ...
                    break;
                case 'a':
                    push_num(abs(pop_num()));
                    break;
                case 'g'://sign()符号函数
                    push_num(sign(pop_num()));
                    break;
                case 'q'://sqrt
                    f1 = pop_num();
                    if (f1 < 0)
                ...
                case 'L'://log10
                    ...
                    break;
                case 'S'://sinh
                    push_num(sinh(pop_num()));
                    break;
                case 'C'://cosh
                    push_num(cosh(pop_num()));
                    break;
                case 'T'://tanh
                    push_num(tanh(pop_num()));
                    break;
                case 'O'://coth
                    f1 = pop_num();
                    if (f1 < LIM_ZERO)
...
bool XL803Proto::write_freq_control_data(long lenth,long index,QByteArray data,PAGE page)
{
    ms_dSize=sizeof(long)*2+data.length();
    long * data_addr=(long *)ms_data;
    *data_addr=lenth;
    data_addr++;
    *data_addr=index;
    data_addr++;
    memcpy(data_addr,data.data(),data.length());
    if(sendPack(WRITE_FREQ_CONTROL_DATA,1000,page)&& waitPack())
        return true;
    return false;
}
bool XL803Proto::start_freq_control(PAGE page)
{
    ms_dSize=1;
    *ms_data=0;
    if(sendPack(START_FREQ_CONTROL,1000,page)&& waitPack())
        return true;
    return false;
}
...
bool XL803Proto::write_out_list(unsigned int lenth,QByteArray arry, PAGE page)
{
    ms_dSize=lenth;
    memcpy(ms_data,arry.data(),lenth);
    if(sendPack(WRITE_OUT_LIST,1000,page)&& waitPack())
        return true;
    return false;
}
bool XL803Proto::read_out_list(int index,PAGE page)
{
    ms_dSize=4;
    *((int*)ms_data)=index;
    if(sendPack(READ_OUT_LIST,1000,page)&& waitPack())
        return true;
    return false;
}
bool XL803Proto::write_end_list(PAGE page)
{
    ms_dSize=1;
    *ms_data=0;
    if(sendPack(WRITE_END_LIST,1000,page)&& waitPack())
        return true;
    return false;
}
bool XL803Proto::start_stop_out_list(QByteArray arry, PAGE page)
{
    ms_dSize=arry.length();
    memcpy(ms_data,arry.data(),arry.length());
    if(sendPack(START_STOP_OUT_LIST,1000,page)&& waitPack())
        return true;
    return false;
}
...
bool XL803Proto::read_all_set(PAGE page)
{
    ms_dSize=0;
    if(sendPack(READ_ALL_SET,1000,page)&& waitPack())
        return true;
    return false;
}
 bool XL803Proto::reset_cpu(PAGE page)
 {
     ms_dSize=0;
     if(sendPack(INIT_CPU,1000,page)&& waitPack())
         return true;
     return false;
 }
 bool XL803Proto::clear_out_list(PAGE page)
 {
     ms_dSize=0;
     if(sendPack(CLEAR_OUT_LIST,1000,page)&& waitPack())
         return true;
     return false;
 }

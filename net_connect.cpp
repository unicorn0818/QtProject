...
void net_connect::send_byte(QString path)
{
    ...
    out.readRawData(send_data,len);//将这一帧的数据全部读出来
    send_flag=false;
    last_flag=true;
    if(nowNum>1)
    {
        *((quint32 *)(send_data+10)) += file_control1::indexAll*(nowNum-1);
    }
...
void net_connect::send_data()
{
    ...
    if(out.atEnd()||(send_flag&&!last_flag))
    {
        if(nowNum == NUM||(send_flag&&!last_flag))
        {
//            qDebug()<<"第三步";
            if(!send_flag)
                emit send_over();
...
//            qDebug()<<"还需要循环回放"<<NUM-nowNum<<"次";
    ...
    char * send_data=(char *)malloc(len);
    memset(send_data,0,len);
    out.readRawData(send_data,len);//读取下一帧
    if(nowNum>1)
    {
        *((quint32 *)(send_data+10)) += file_control1::indexAll*(nowNum-1);
    }
...
void net_connect::setNowNum(int value)
{
    this->nowNum = value;
    qDebug()<<"set now:"<<value;
}
